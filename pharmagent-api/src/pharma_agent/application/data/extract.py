...
from pharma_agent.domain.philosopher import Philosopher, PhilosopherExtract
...
def extract(philosopher: Philosopher, extract_urls: list[str]) -> list[Document]:
    ...
    if philosopher.id == "jnj":
        from pathlib import Path
        jnj_path = Path(__file__).parents[4] / "data" / "jnj_content.txt"
        if jnj_path.exists():
            with open(jnj_path, "r", encoding="utf-8") as f:
                content = f.read()
            docs.append(
                Document(
                    ...
                    metadata={
                        ...
                        "philosopher_id": "jnj",
                        "philosopher_name": "Johnson & Johnson Assistant",
                    },
                )
            )
            return docs
...
def extract_wikipedia(philosopher: Philosopher) -> list[Document]:
    ...
    """
    import wikipedia
    wikipedia.set_user_agent("pharma_agent-course/1.0 (https://github.com/neural-maze/pharma_agent-course; support@theneuralmaze.com) wikipedia-python/1.4.0")
...
if __name__ == "__main__":
    jnj = PhilosopherFactory().get_philosopher("jnj")
    docs = extract(jnj, ["https://www.jnj.com/"])
    print(docs)
