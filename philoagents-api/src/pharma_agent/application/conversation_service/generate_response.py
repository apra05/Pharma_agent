...
from pharma_agent.application.conversation_service.workflow.graph import (
    ...
    try:
        async with AsyncMongoDBSaver.from_conn_string(
            ...
            async for chunk in graph.astream(
                ...
                if node_name == "conversation_node" and isinstance(
                    chunk[0], AIMessageChunk
                ):
                    yield chunk[0].content
                elif (
                    node_name == "guardrail_node"
                    and isinstance(chunk[0], AIMessage)
                    and not isinstance(chunk[0], AIMessageChunk)
                ):
                    yield chunk[0].content
...
async def get_latest_conversation_state(thread_id: str) -> PhilosopherState | None:
    """Retrieves the latest state for a given thread ID from the checkpointer.

    Args:
        thread_id: The thread ID of the conversation to retrieve.

    Returns:
        PhilosopherState | None: The latest state if found, otherwise None.
    """
    graph_builder = create_workflow_graph()
    try:
        async with AsyncMongoDBSaver.from_conn_string(
            ...
            if state and state.values:
                return PhilosopherState(**state.values)
    except Exception as e:
        from loguru import logger
        logger.warning(f"Failed to retrieve latest conversation state for thread {thread_id}: {e}")
    return None
